...
# ─────────────────────────────────────────────────────────────────
# FRIENDS GROUP QUIZ
# FIX #6: Corrected player-turn advancement logic to prevent double
# rerun and ensure done state is properly set before rerun.
# ─────────────────────────────────────────────────────────────────
def page_friends():
    u = st.session_state.user
    ...
    if grp is not None and grp.get("done"):
        ...
        <div style="text-align:center;background:linear-gradient(135deg,#FFF8E7,#FFFBF0);
            ...
            <div style="font-size:20px;font-weight:800;color:#A07820">
    ...
    if grp is not None and not grp.get("done"):
        ...
        # FIX #6: Handle player completion in a single rerun path
        if qidx >= total_q:
            next_player = pidx + 1
            if next_player >= len(grp["players"]):
                grp["done"] = True
            else:
                grp["current_player"] = next_player
            st.session_state.group_session = grp
            st.rerun()
            return
        ...
        st.markdown(f"""
        <div style="background:linear-gradient(135deg,rgba(124,58,237,0.09),rgba(167,139,250,0.09));border-radius:14px;
